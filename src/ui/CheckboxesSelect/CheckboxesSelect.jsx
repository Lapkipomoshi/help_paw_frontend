...
const CheckboxesSelect = ({ caption, setInputValue }) => {
  ...
  useEffect(() => {
    const newSet = [];
    if (isCat) {
      newSet.push(catType.caption);
    }
    if (isDog) {
      newSet.push(dogType.caption);
    }
    if (isParrot) {
      newSet.push(parrotType.caption);
    }
    if (isHamster) {
      newSet.push(hamsterType.caption);
    }
    setItems([...newSet]);
    const newInputValue = [];
    if (isCat) {
      newInputValue.push(catType.name);
    }
    if (isDog) {
      newInputValue.push(dogType.name);
    }
    if (isParrot) {
      newInputValue.push(parrotType.name);
    }
    if (isHamster) {
      newInputValue.push(hamsterType.name);
    }
    ...
    <li className='checkboxes-select'>
      ...
      <div className='checkboxes-select__multiselect'>
        <div className='checkboxes-select__select-box'>
              ...
              items.map((item, index) => {
                return (
                  ...
                  <li className='checkboxes-select__item' key={index}>
                    ...
                    <button
                      className='checkboxes-select__delete'
                      type='button'
                      data-type={item}
                      onClick={(e) => {
                        handleDeleteType(e);
          ...
          <button
            className='checkboxes-select__arrow'
            type='button'
            onClick={() => {
        ...
        <div className={`checkboxes-select__checkboxes ${openedSelect && 'checkboxes-select__checkboxes_opened'}`}>
          <label className='checkboxes-select__check-label' htmlFor={catType.name}>
            <input className='checkboxes-select__checkbox' type='checkbox' name={catType.name} id={catType.name} onChange={handleCheckbox} />
            {catType.caption}
          </label>
          <label className='checkboxes-select__check-label' htmlFor={dogType.name}>
            <input className='checkboxes-select__checkbox' type='checkbox' name={dogType.name} id={dogType.name} onChange={handleCheckbox} />
            {dogType.caption}
          </label>
          <label className='checkboxes-select__check-label' htmlFor={parrotType.name}>
            <input className='checkboxes-select__checkbox' type='checkbox' name={parrotType.name} id={parrotType.name} onChange={handleCheckbox} />
            {parrotType.caption}
          </label>
          <label className='checkboxes-select__check-label' htmlFor={hamsterType.name}>
            <input className='checkboxes-select__checkbox' type='checkbox' name={hamsterType.name} id={hamsterType.name} onChange={handleCheckbox} />
            {hamsterType.caption}
          </label>
        </div>
      </div>

...
const AddShelterPage = ({ currentUser, openPopup, setPopupImage, setMessage }) => {
  ...
  const handleSubmitForm = (evt) => {
    ...
    if (step === 1) {
      ...
      addShelterApi
        .postShelter(token, { ...shelterOwner, ...shelter })
        .then(() => {
  ...
  return (
    <MainContainer>
      <main className='main add-shelter'>
        ...
        <p className='add-shelter__subtitle'>
          {step === 1 ? 'Чтобы добавить приют, заполните, пожалуйста, данные о владельце приюта.' : 'Заполните, пожалуйста, информацию о приюте.'}
        </p>
        <form className='add-shelter-form' name='add-shelter' onSubmit={handleSubmitForm} noValidate>
          {step === 1 && <OwnerStep currentUser={currentUser} setShelterOwner={setShelterOwner} />}
          {step === 2 && (
            <ShelterStep
              handleBack={() => {
                setStep(1);
